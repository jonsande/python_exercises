'''
Using the Python language, have the function ArithGeo(arr) take 
...
def arith_geo(array):
    ...
    for i in range(len(array)-1):
        geo_list.append(array[i+1]/array[i])
        arith_list.append(array[i+1]-array[i])

    if len(set(geo_list)) == 1:
        return "Geometric"
    elif len(set(arith_list)) == 1:
        return "Arithmetic"
    else:
        return -1