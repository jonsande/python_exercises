...
write an algorithm that runs in O(n) time. The challenge is to find all the 
pairs of two integers in an unsorted array that sum up to a given S.
...
def two_sum(array, sum):
    ...
    for i in range(0, len(array)):
        for e in range(i+1, len(array)):
            #print(f"{array[i]}, {array[e]}")
            if array[i] + array[e] == sum:
                #print(f"Pareja encontrada: {array[i]}, {array[e]}")
                output.append([array[i], array[e]])
    return output
...
def two_sum(array, sum):
    ...
    for i in array:
        if sum - i in array and i not in dict.values():
            dict[i] = sum - i
